...
import { removeService, changeServiceField, changeService, cancelChangeService, filterService, filterCopyState } from '../redux/actionCreator';
import store from '../redux/store'

function ServiceList() {
    const item = useSelector(state => state.serviceList);
    const filterItems = useSelector(state => state.serviceFilterList)
    const filterFlag = useSelector(state => state.serviceFilter.filterFlag)
    const dispatch = useDispatch();

   
    let items 
    filterFlag?  items = filterItems : items = item
